...
| B7 | `Host ovh-app` in `~/.ssh/config` has no `HostName` | The alias cannot connect; `ovh1-ts` must be used instead. Latent trap for anything scripted against `ovh-app`. | **OPEN** (outside this repo). |
