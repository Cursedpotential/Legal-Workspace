...
| `michigan-court-rules_2026-07-31.md` | **Structured markdown** extraction of the above (`pymupdf4llm` — headings, tables, section numbers preserved) | — | 2.5 MB |
...
| `michigan-rules-of-evidence_2026-01-28.md` | Structured markdown extraction of the above | — | 73 KB |

The **markdown** is what the plugin ships and what a model reads — it preserves the
document's heading hierarchy, tables, and section numbering, unlike a flat text dump.
The **PDFs** are the binary originals, kept here (not in the plugin) and re-fetchable.
...
# structured markdown (what the plugin ships):
python3 -c "import pymupdf4llm,sys; open(sys.argv[2],'w').write(pymupdf4llm.to_markdown(sys.argv[1]))" \
  michigan-court-rules.pdf michigan-court-rules.md
```

`pymupdf4llm` (`pip install pymupdf4llm`) produces structured markdown; `fetch-sources.sh`
uses it automatically when present, falling back to `pdftotext -layout` (from
`poppler-utils`) if it is not installed.
